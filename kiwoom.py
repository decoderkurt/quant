...
화면번호_주식분봉차트조회_PREFIX = 'CMON'  # 종목코드 붙여서 화면번호 구성
화면번호_주식일봉차트조회_PREFIX = 'CDAY'  # 종목코드 붙여서 화면번호 구성
...
class Kiwoom(QObject):
    ...
    def kiwoom_OnEventConnect(self, nErrCode, **kwargs):
        ...
        if nErrCode == 0:
            ...
            logger.info('사용자 정보교환 실패')
        elif nErrCode == 101:
            logger.info('서버접속 실패')
        elif nErrCode == 102:
            logger.info('버전처리 실패')
        else:
            logger.info('알수 없는 결과')
    ...
    def kiwoom_TR_OPT10081_주식일봉차트조회(self, strCode, strDate, tick=1, fix=1, nPrevNext=0, **kwargs):
        '''주식일봉차트조회
        :param strCode: 종목코드
        :param strDate: 기준일자 = YYYYMMDD (20160101 연도4자리, 월 2자리, 일 2자리 형식)
        :param fix: 수정주가구분 (0 or 1, 수신데이터 1:유상증자, 2:무상증자, 4:배당락, 8:액면분할, 16:액면병합, 32:기업합병, 64:감자, 256:권리락)
        :param nPrevNext:
        :param kwargs:
        :return:
        '''
        res = self.kiwoom_SetInputValue('종목코드', strCode)
        res = self.kiwoom_SetInputValue('기준일자', strDate)
        res = self.kiwoom_SetInputValue('수정주가구분', str(fix))
        res = self.kiwoom_CommRqData('주식일봉차트조회', 'opt10081', nPrevNext, 화면번호_주식일봉차트조회_PREFIX + strCode)
    ...
    # 조회 관련 콜백함수
    # -------------------------------------
    def cb_주식분봉차트조회(self, sScrNo, sRQName, sTRCode, sRecordName, sPreNext, nDataLength, sErrorCode, sMessage,
                               sSPlmMsg, **kwargs):
        cnt = self.kiwoom_GetRepeatCnt(sTRCode, sRQName)
    ...
    def kiwoom_OnReceiveRealData(self, sCode, sRealType, sRealData, **kwargs):
        ...
        :return:
        '''
        logger.info('REAL: %s %s %s' % (sCode, sRealType, sRealData))
    ...
    def kiwoom_OnReceiveTrCondition(self, sScrNo, strCodeList, strConditionName, nIndex, nNext, **kwargs):
        ...
        logger.info('조건검색 결과: %s' % (list_str_code,))
    ...
    def kiwoom_OnReceiveRealCondition(self, strCode, strType, strConditionName, strConditionIndex, **kwargs):
        ...
        :return:
        '''
        logger.info('실시간 조건검색: %s %s %s %s' % (strCode, strType, strConditionName, strConditionIndex))
        if strType == 'I':
            # 모니터링 종목 리스트에 추가
    ...
    def kiwoom_SendOrder(self, sRQName, sScreenNo, sAccNo, nOrderType, sCode, nQty, nPrice, sHogaGb, sOrgOrderNo,
        ...
        :return:
        '''
        logger.info('주문: %s %s %s %s %s %s %s %s %s' % (
    ...
    def kiwoom_OnReceiveMsg(self, sScrNo, sRQName, sTrCode, sMsg, **kwargs):
        ...
        :return:
        '''
        logger.info('주문/잔고: %s %s %s %s' % (sScrNo, sRQName, sTrCode, sMsg))
